...
public class PokemonData {
    ...
    public static Move getMove(String moveName) throws NoSuchElementException {
        if (moves.get(moveName) == null) {
            throw new NoSuchElementException("Move " + moveName + " does not exist");
        }
        return moves.get(moveName);
    }

    public static Pokemon getPokemon(String pokemonName) throws NoSuchElementException {
        if (pokemons.get(pokemonName) == null) {
            throw new NoSuchElementException("Pokemon " + pokemonName + " does not exist");
        }
        return pokemons.get(pokemonName);
    }

    public static boolean canBeShadow(String pokemonName) throws NoSuchElementException {
        if (pokemons.get(pokemonName) == null) {
            throw new NoSuchElementException("Pokemon " + pokemonName + " does not exist");
        }
    ...
    */
    ...
    public void testAttributeCalc(String pokemon, Stats iv, double level) {
        Pokemon attacker1 = pokemons.get("Reshiram").createInstance(iv, level, false);
        Pokemon attacker2 = pokemons.get("Gengar").createInstance(iv, level, false);
        Pokemon defender = pokemons.get("Kartana").createInstance(iv, level, false);
