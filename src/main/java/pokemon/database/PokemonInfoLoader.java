...
public class PokemonInfoLoader {
    ...
    public static void loadCPMult() {
        ...
        try {
            ...
            Scanner input = new Scanner(new File("src/main/resources/CPMultipliers.in"));
                    ...
                    if (inputLine.contains("{")) {
                        while ((inputLine = in.readLine()) != null && !inputLine.contains("}")) {
                            ...
                            if (attributeName.contains("base_attack")) {
                                newPokemon.baseStats.attack = Integer.parseInt(attributeValue.substring(0, attributeValue.indexOf(",")));
                            }
                            else if (attributeName.contains("base_defense")) {
                                newPokemon.baseStats.defense = Integer.parseInt(attributeValue.substring(0, attributeValue.indexOf(",")));

                            }
                            else if (attributeName.contains("base_stamina")) {
                                newPokemon.baseStats.stamina = Integer.parseInt(attributeValue.substring(0, attributeValue.indexOf(",")));
                            }
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                while ((inputLine = in.readLine()) != null) {
                    if (inputLine.contains("{")) {
                        ...
                        String form = "";
                        int ID;
                        List<MoveElite> fastMoves = new ArrayList<MoveElite>();
                        List<MoveElite> charMoves = new ArrayList<MoveElite>();
                        while ((inputLine = in.readLine()) != null && !inputLine.contains("}")) {
                            inputLine = inputLine.trim();
                            int firstIndex;
                            if (inputLine.contains("pokemon_name")) {
                                ...
                                while ((inputLine = in.readLine()) != null && !inputLine.contains("]")) {
                                    charMoves.add(new MoveElite(removeQuotes(inputLine, 0), elite));
                                }
                            }
                                ...
                                boolean elite = inputLine.contains("elite");
                                while ((inputLine = in.readLine()) != null && !inputLine.contains("]")) {
                                    fastMoves.add(new MoveElite(removeQuotes(inputLine, 0), elite));
                                }
                            }
                        }
                        if (pokemon.containsKey(Pokemon.getNameForm(pokeName, form))) {
                ...
                while ((inputLine = in.readLine()) != null) {
                    if (inputLine.contains("{")) {
                        ...
                        while ((inputLine = in.readLine()) != null && !inputLine.contains("}")) {
                            ...
                            if (attribute[0].contains("\"stats\"")) {
                                while ((inputLine = in.readLine()) != null && !inputLine.contains("}")) {
                                    ...
                                    if (stat[0].contains("attack")) {
                                        newMega.baseStats.attack = Integer.parseInt(stat[1].substring(0, stat[1].indexOf(",")));
                                    }
                                    if (stat[0].contains("defense")) {
                                        newMega.baseStats.defense = Integer.parseInt(stat[1].substring(0, stat[1].indexOf(",")));
                                    }
                                    if (stat[0].contains("stamina")) {
                                        newMega.baseStats.stamina = Integer.parseInt(stat[1]);
                                    }
                                }
                            }
                            ...
                            if (attribute[0].contains("pokemon_id")) {
                                attribute[1] = attribute[1].trim();
                                newMega.ID = Integer.parseInt(attribute[1].substring(0, attribute[1].indexOf(",")));
                            }
                        }
